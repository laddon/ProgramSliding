	...
	ensures SliceOf(S,V) == res 
{
	...
	forall slide | slide in slidesSV ensures VarSlideOf(SV, SV', slide) in varSlidesSV {
		...
		if slide in finalDefSlides(S, slideDG, cfg, V) // for example sum2,sum5
		{
		...
		else // for example i1,i7
		{
			assert varSlide in varSlidesSV by {
				assert exists Sn: VarSlide :: Sn.0 in V' && VarSlideDGReachable(slideDG, varSlide/*i1*/, Sn/*sum4*/, varSlideDG'.1) by {
					// i1 --> sum5 --> sum4 , sum5 is pre of sum4 via phi only
					var finalDefSlide :| finalDefSlide in finalDefSlides(S, slideDG, cfg, V) && SlideDGReachable(slideDG, slide, finalDefSlide, slideDG.1);
					...
					assert VarSlideDGReachable(slideDG, varSlide, finalDefVarSlide, varSlideDG'.1);

					var instancesOfFinalDefSlideSeq := vsSSA.getInstancesOfVaribleFunc(finalDefSlide.1);
					var instancesOfFinalDefSlide := setOf(instancesOfFinalDefSlide);
					var v' := instancesOfFinalDefSlide * V';
					var slideOfV' :| slideOfV'.0 == v';// varSlideOf(v'); // TODO

					if slideOfV'.1 == Regular
					{
						assert slideOfV' == finalDefVarSlide;
						assert slideOfV'.0 in V' && VarSlideDGReachable(slideDG, varSlide/*i1*/, slideOfV'/*sum5*/, varSlideDG'.1);
						// Done
					}
					else // Phi
					{
						assert slideOfV' != finalDefVarSlide; // צריך להוכיח מסלול ביניהם
						assert VarSlideDGReachable(varSlideDG, finalDefVarSlide, slideOfV', varSlideDG'.1);
						assert finalDefVarSlide in RegularPredecessorSlides(slideOfV', varSlideDG) by {
							...
							L4(X, vsSSA, v', V', instancesOfFinalDefSlide, finalDefSlide, S, slideDG, cfg, V, slide, slidesSV, slideOfV', finalDefVarSlides);
						}
					}
	...
	forall varSlide | varSlide in varSlidesSV && varSlide.1 == Regular ensures SlideOf(SV, SV', varSlide) in slidesSV {
		assert varSlide in varSlidesSV;
		
		var slide := SlideOf(SV, SV', varSlide);
		
		assert slide in slidesSV by {
			assert slide in finalDefSlides(S, slideDG, cfg, V) || (exists Sn :: Sn in finalDefSlides(S, slideDG, cfg, V) && SlideDGReachable(slideDG, slide, Sn, slideDG.1)) by {
...
lemma {:verify false}L4(X: set<Variable>, vsSSA: VariablesSSA, v': Variable, V': set<Variable>, instancesOfFinalDefSlide: set<Variable>,
		 finalDefSlide: Slide, S: Statement, slideDG: SlideDG, cfg: CFG, V: set<Variable>, slide: Slide, slidesSV: set<Slide>,
		 slideOfV': varSlide, finalDefVarSlides: set<VarSlide>)
	...
	requires instancesOfFinalDefSlide == vsSSA.getInstancesOfVaribleFunc(finalDefSlide.1)
	...
	ensures forall x,i :: x in X && i in vsSSA.getInstancesOfVaribleFunc(x) && varslideof(i)/*TODO*/ in finalDefVarSlides ==>
																			i in RegularPredecessorSlides(slideOfV', varSlideDG)
...
function {:verify false}RegularPredecessorSlides(varSlide: VarSlide, varSlideDG: VarSlideDG): set<VarSlide>
{
	var predecessors := varSlideDG.2[varSlide];

	(set vSlide | vSlide in predecessors && vSlide.1 == Regular) + 
	(set vSlide1, vSlide2 | vSlide1 in predecessors && vSlide1.1 == Phi && vSlide2 in RegularPredecessorSlides(vSlide1, varSlideDG) :: vSlide2)

/*
	if varSlideDG.2[varSlide] == {} then {} 
	else
	{
		var predecessors := varSlideDG.2[varSlide];
		var res := {};
		...
		while |predecessors| > 0
		{
			var vSlide :| vSlide in predecessors;
			predecessors := predecessors - {vSlide};
			if vSlide.1 == Regular
			{
				res := res + {vSlide};
			}
			...
			{
				res := RegularPredecessorSlide(vSlide, varSlideDG);
			}
		}
	}*/
}
...
function {:verify false}allSlides(S: Statement) : set<Slide>
	reads *
	requires Valid(S)
	requires Core(S)
{
...
predicate {:verify false}Substatement(S': Statement, S: Statement)
	reads *
	requires Valid(S')
	requires Valid(S)
	requires Core(S')
...
predicate {:verify false}assignmentsMatch(LHS: seq<Variable>, LHS': seq<Variable>, RHS: seq<Expression>, RHS': seq<Expression>)
...
function {:verify false}findVariableInSeq(v: Variable, vars: seq<Variable>): int
	requires v in vars
